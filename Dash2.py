...
df = pd.read_csv('Historico.zip', compression='zip')
			...
			if grafico_tipo == v1 and grafico_tipo2 == v2 and grafico_tipo3 == v3:
				...
				for month in range(12):
				    ...
				    if calendar.monthrange(year, month+1)[1] == 28:
				        ...
				        exec(f'fig{month+1}.update_layout(title_text = f"Produtividade mês de {m[month]}", xaxis=dict(range=[0, 10]), yaxis=dict(range=[0, 10]))')
